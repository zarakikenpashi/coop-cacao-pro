...
const crumbs = () => {
  ...
  return [
    { label: 'MLexpert', active: false },
    ...parts.map((label, i) => ({ label, active: i === parts.length - 1 })),
  ]
}
...
<template>
  ...
  <header
    ref="topbarRef"
    class="w-full h-12 bg-card border-b border-border flex items-center justify-between px-4 relative shrink-0 z-30"
  >
    <!-- ── GAUCHE : hamburger + breadcrumb ───────────────────────── -->
    <div class="flex items-center gap-2 min-w-0">
      <!-- Hamburger mobile -->
      <button
        class="md:hidden w-8 h-8 flex items-center justify-center rounded-md text-muted-foreground hover:bg-muted hover:text-foreground transition-colors mr-1 shrink-0"
      ...
      <nav class="flex items-center gap-1 text-[12px] min-w-0">
        <template v-for="(crumb, i) in crumbs()" :key="i">
          ...
          <span v-if="i < crumbs().length - 1" class="text-muted-foreground/40 text-[10px] shrink-0"
            >›</span
          >
        </template>
      </nav>
    ...
    <div class="flex items-center gap-1.5 shrink-0">
      ...
      <button
        class="relative w-8 h-8 flex items-center justify-center rounded-md text-muted-foreground hover:bg-muted hover:text-foreground transition-colors"
      >
      ...
      <div class="relative">
        <button
          class="flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-muted transition-colors cursor-pointer"
        ...
        <Transition name="dropdown">
          <div
            v-if="userMenuOpen"
            class="absolute right-0 top-full mt-2 w-44 bg-card border border-border rounded-xl shadow-lg overflow-hidden z-50"
          >
            <!-- Header -->
            ...
            <div class="p-1">
              <button
                ...
                :key="item.label"
                class="w-full flex items-center gap-2.5 px-2.5 py-2 text-[12px] text-foreground rounded-lg hover:bg-muted transition-colors"
              >
                <!-- Mini icône inline -->
                <svg
                  width="13"
                ...
                >
                  ...
                  <path
                    ...
                    stroke-linejoin="round"
                    stroke-width="2"
                    d="M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6zM19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06A1.65 1.65 0 0 0 15 19.4a1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"
                  />
                  <path
                    v-if="item.icon === 'help'"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    stroke-width="2"
                    d="M9 9a3 3 0 0 1 6 0c0 2-3 3-3 3m0 4h.01"
                  />
                </svg>
                {{ item.label }}
              </button>
              ...
              <button
                class="w-full flex items-center gap-2.5 px-2.5 py-2 text-[12px] text-destructive rounded-lg hover:bg-destructive/8 transition-colors"
              >
                <svg
                  width="13"
                  height="13"
