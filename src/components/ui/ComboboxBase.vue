...
<style scoped>
.cb-root {
  /* 🌿 On map sur ton design system global */
  --cb-primary: var(--primary);
  --cb-fg: var(--foreground);
  --cb-bg: var(--card);
  --cb-muted: var(--muted);
  --cb-muted-fg: var(--muted-foreground);
  --cb-border: var(--border);
  --cb-ring: rgba(31, 122, 62, 0.15); /* vert focus */
...
.cb-item--active {
  background: rgba(244, 196, 48, 0.15); /* jaune cacao */
}
.cb-trigger__label {
  flex: 1;
  text-align: left;
...
.cb-popover {
  ...
  border: 1px solid var(--cb-border);
  border-radius: var(--cb-radius);
  box-shadow: 0 8px 24px rgba(15, 46, 28, 0.12);
  z-index: 50;
  overflow: hidden;
}
.cb-popover--bottom {
...
.cb-item--selected {
  background: rgba(31, 122, 62, 0.08); /* vert léger */
  color: var(--cb-primary);
  font-weight: 500;
}
.cb-item__label {
